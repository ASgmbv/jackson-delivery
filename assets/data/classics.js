export default {
  title: "Classics",
  items: [
    {
      title: "Regular",
      description:
        "Two eggs any style served with choice of bacon or sausage, hasbrowns and OSM toast",
      price: 12.25,
      image: "/food/regular.jpg",
      options: [
        {
          title: "How would you like your eggs to be cooked?",
          slug: "Eggs Type",
          values: [
            {
              value: "Over Easy",
              extra: 0,
            },
            {
              value: "Over Medium",
              extra: 0,
            },
            {
              value: "Over Hard",
              extra: 0,
            },
            {
              value: "Scramble",
              extra: 0,
            },
            {
              value: "Scramble Soft",
              extra: 0,
            },
            {
              value: "Scramble Hard",
              extra: 0,
            },
            {
              value: "Egg White",
              extra: 0,
            },
            {
              value: "Egg Beaters",
              extra: 0,
            },
            {
              value: "Sunny Side Up",
              extra: 0,
            },
            {
              value: "No Eggs",
              extra: 0,
            },
          ],
        },
        {
          title:
            "Would you like to substitute your toast for additional charge?",
          slug: "Toast Substitute",
          values: [
            {
              value: "No",
              extra: 0,
            },
            {
              value: "OSM Bread",
              extra: 0,
            },
            {
              value: "White Bread",
              extra: 1.25,
            },
            {
              value: "Cinnamon Raisin",
              extra: 1.25,
            },
            {
              value: "Butter Croissant",
              extra: 1.75,
            },
            {
              value: "Rustic Eye Bread",
              extra: 0,
            },
          ],
        },
        {
          title: "What would you like for your order?",
          slug: "Meat Type",
          values: [
            {
              value: "Bacon",
              extra: 0,
            },
            {
              value: "Sausage",
              extra: 0,
            },
            {
              value: "Ham Steak",
              extra: 4.95,
            },
            {
              value: "Crispy Bacon",
              extra: 0,
            },
            {
              value: "Deli Ham",
              extra: 0,
            },
            {
              value: "No Meat",
              extra: 0,
            },
          ],
        },
      ],
    },
    {
      title: "The Signature",
      description:
        "Two eggs any style with one OSM pancake or butter pancake and your choice of bacon or sausage",
      price: 12.5,
      image: "/food/signature.jpg",
      options: [
        {
          title: "How would you like your eggs to be cooked?",
          slug: "Eggs Type",
          values: [
            {
              value: "Over Easy",
              extra: 0,
            },
            {
              value: "Over Medium",
              extra: 0,
            },
            {
              value: "Over Hard",
              extra: 0,
            },
            {
              value: "Scramble",
              extra: 0,
            },
            {
              value: "Scramble Soft",
              extra: 0,
            },
            {
              value: "Scramble Hard",
              extra: 0,
            },
            {
              value: "Egg White",
              extra: 0,
            },
            {
              value: "Egg Beaters",
              extra: 0,
            },
            {
              value: "Sunny Side Up",
              extra: 0,
            },
            {
              value: "No Eggs",
              extra: 0,
            },
          ],
        },
        {
          title: "What kind of pancake would you like?",
          slug: "Pancake Type",
          values: [
            {
              value: "Buttermilk Pancake",
              extra: 0,
            },
            {
              value: "OSM Pancake",
              extra: 0,
            },
          ],
        },
        {
          title: "What would you like for your order?",
          slug: "Meat Type",
          values: [
            {
              value: "Bacon",
              extra: 0,
            },
            {
              value: "Sausage",
              extra: 0,
            },
            {
              value: "Ham Steak",
              extra: 4.95,
            },
            {
              value: "Crispy Bacon",
              extra: 0,
            },
            {
              value: "Deli Ham",
              extra: 0,
            },
            {
              value: "No Meat",
              extra: 0,
            },
          ],
        },
        {
          title: "What kind of syrup would you like?",
          slug: "Syrup Type",
          values: [
            {
              value: "Pure Marple Syrup",
              extra: 0,
            },
            {
              value: "Sugar Free Syrup",
              extra: 0,
            },
          ],
        },
      ],
    },
    {
      title: "Biscuits & Gravy",
      description:
        "Our homemade buttermilk biscuits and hash browns smothered with pepper sausage country gravy",
      price: 10.5,
      image: "/food/gravy.jpg",
      options: [
        {
          title: "Would you like crispy hush browns?",
          slug: "Hush Brown",
          values: [
            {
              value: "No",
              extra: 0,
            },
            {
              value: "Yes",
              extra: 0,
            },
          ],
        },
      ],
    },
    {
      title: "Steak & Eggs",
      price: 18.5,
      description:
        "Wyoming Gourmet Beef all natural grilled 6-ounce steak served with two eggs any style, hash browns OSM toast",
      image: "/food/steak.jpg",
      options: [
        {
          title: "How would you like your eggs to be cooked?",
          slug: "Eggs Type",
          values: [
            {
              value: "Over Easy",
              extra: 0,
            },
            {
              value: "Over Medium",
              extra: 0,
            },
            {
              value: "Over Hard",
              extra: 0,
            },
            {
              value: "Scramble",
              extra: 0,
            },
            {
              value: "Scramble Soft",
              extra: 0,
            },
            {
              value: "Scramble Hard",
              extra: 0,
            },
            {
              value: "Egg White",
              extra: 0,
            },
            {
              value: "Egg Beaters",
              extra: 0,
            },
            {
              value: "Sunny Side Up",
              extra: 0,
            },
            {
              value: "No Eggs",
              extra: 0,
            },
          ],
        },
        {
          title: "How would you like your steak to be cooked?",
          slug: "Steak Type",
          values: [
            {
              value: "Rare",
              extra: 0,
            },
            {
              value: "Medium Rare",
              extra: 0,
            },
            {
              value: "Medium Well",
              extra: 0,
            },
            {
              value: "Well Done",
              extra: 0,
            },
          ],
        },
        {
          title: "Would you substitute your toast for additional charge?",
          slug: "Toast Substitute",
          values: [
            {
              value: "No",
              extra: 0,
            },
            {
              value: "OSM Bread",
              extra: 0,
            },
            {
              value: "White Bread",
              extra: 1.25,
            },
            {
              value: "Cinnamon Raisin",
              extra: 1.25,
            },
            {
              value: "Butter Croissant",
              extra: 1.75,
            },
            {
              value: "Rustic Eye Bread",
              extra: 0,
            },
          ],
        },
      ],
    },
  ],
};
